...
const unsigned long doubleTapCooldown = 2000; // 2 seconds cooldown
...
#define UPDATES_PER_SECOND 1 // 250, was 50
...
// Add this I2C verification
bool connectADXL345(int maxRetries = 5) {
  for (int retry = 0; retry < maxRetries; retry++) {
    Serial.print("Attempting ADXL345 connection... try ");
  ...
  if (error == 0) {
    Serial.println("ADXL345 found at address 0x53 ✓");
    Serial.println("I2C Pins: SDA=A4, SCL=A5 (on Uno)");
    return true;
  } 
  delay(1000); // wait 1 sec before retry
}

  // ADXL not found after loop, program halt
  Serial.println("ADXL345 NOT found! Check wiring!");
  Serial.println("Program HALTED - fix wiring and restart");
...
void loop() // run over and over again
{
  ...
  if(debugMode) {
    Serial.print("\tXPin,YPin =");
    Serial.print(digitalRead(XtogglePin));
    Serial.print(",");
    Serial.println(digitalRead(YtogglePin)); 
    Serial.print("/tHue = ");
  ...
  if (tapDetectionToggle)
  {
    ...
    if (adxl.triggered(interrupts, ADXL345_DOUBLE_TAP))
    {
      ...
      if (currentTime - lastDoubleTapTime > doubleTapCooldown) {
        if (debugMode)
